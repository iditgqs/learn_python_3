...
class HTTPServer(object):
    ...
    def start_response(self, status, heads):
        """"""
        response_head = response_head_p = "HTTP/1.1 " + status + "\r\n"
        for head in heads:
            response_head += "%s: %s\r\n"%head
    ...
    def handle_client(self, client_socket_info):
        ...
        # 处理动态资源
        if http_url.endswith(".py"):
            try:
                # 导入py模块
                print(http_url[1:-3])
                mo =  __import__(http_url[1:-3])    
                # 根据wsgi协议
                env = {}
                http_source_content = mo.application(env, self.start_response)
            except ImportError: # 资源不存在
                # 返回http响应
                ...
                self.response_head = response_head 
                http_source_content = " not found "
                 
            respnose = self.response_head + http_source_content 
            response = respnose.encode("utf-8")
